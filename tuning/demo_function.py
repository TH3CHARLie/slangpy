"""Demo: function-level autotuning via FunctionNode.with_settings().

Mirrors demo.py but drives specialization through the SlangPy kernel generator's
link-time type bindings, keyed on the FunctionNode chain's slangpy_signature.
The spy.Module is built once; each config becomes a distinct CallDataCache entry.
"""
...
# Set up the tuner purely for its search model and space discovery.
tuner = Tuner(model=ExhaustiveSearch())
space = tuner.setup(device, raw_module)
print(space)
print()

# Build the spy.Module once and grab the function node once.
module = spy.Module(raw_module)
func = module.transform_reduce
...
while not tuner.is_complete():
    ...
    variant_func = func.with_settings(config)

    # Warm up (first call compiles the kernel)
    variant_func(*test_values)

    # Measure
    t0 = time.perf_counter()
    result = variant_func(*test_values)
